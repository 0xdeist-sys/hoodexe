...
const demoAccrued=[120,95,150,60,80,130,110,70,90,145,75,65,50];
const cowGrid=document.getElementById("cowGrid");
cows.forEach((c,i)=>{
  const card=document.createElement("article");
  card.className="cow-card";
  const pct=Math.min(100,35+(i*7)%66);
  card.innerHTML=`
    <div class="img-wrap">
      <img src="${c[1]}" alt="MooPunk #${c[0]}">
      <span class="id-badge">#${c[0]}</span>
    </div>
    <div class="cow-info">
      <h3>MooPunk #${c[0]}</h3>
      <div class="milk-row"><span>DAILY</span><b>10 MILK</b></div>
      <div class="milk-row"><span>UNCLAIMED</span><b>${demoAccrued[i]} MILK</b></div>
      <div class="mini-bar"><i style="width:${pct}%"></i></div>
      <span class="ready">● PRODUCING</span>
    </div>`;
  card.addEventListener("click",()=>openCow(c));
  cowGrid.appendChild(card);
});

collectAll.onclick=()=>{
  balance+=130;
  milkBalance.textContent=balance.toLocaleString("en-US");
  cowsUnclaimed.textContent=balance.toLocaleString("en-US")+" MILK";
  const pop=document.createElement("div");
  pop.className="collect-pop";
  pop.textContent="+130 MILK COLLECTED";
